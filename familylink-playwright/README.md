...
- 🖥️ **noVNC Web Access**: Browser runs in container, accessible via noVNC in your browser (port 6080)
...
3. **Authenticate**:
   ...
   - **Open noVNC in your browser** to see and interact with the browser:
     ...
     - **Password**: `familylink`
     - No additional software needed - works in any modern browser
...
3. **noVNC + websockify** provides web-based access to the virtual display (port 6080)
4. User opens `http://<host>:6080/vnc.html` in their browser to interact
...
**Why noVNC is needed**: The browser runs inside the Docker container. noVNC allows you to see and interact with it from your web browser.
...
**Solution**: Open noVNC in your browser to see and interact with the browser:

1. **URL**: `http://[YOUR_HA_IP]:6080/vnc.html`
2. **Password**: `familylink`
3. Works in any modern web browser - no additional software needed

**If noVNC connection fails**:
- Check add-on logs: `Supervisor` → `Google Family Link Auth` → `Log`
- Look for: `Starting noVNC on port 6080...`
